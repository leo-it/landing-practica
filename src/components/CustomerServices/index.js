import { Box } from '@mui/material'
import React from 'react'

export const CustomerServices = () => {
  return (
    <Box
    sx={{
      height: {xs:"60vh",md:"105vh"},
      display: "flex",
      justifyContent: "center",
      mb: "40px",
    }}
  >
    <Box
      sx={{
        width: "45%",
        mb: "20px",
        mt: "50px",
        position: "relative",
      }}
    >
      <Box sx={{ height: "100px", mb:6 }}>

        <Box sx={{ position: "absolute", top: "70px", left: "-10vw" }}>
          {" "}
          <img
            src="https://th.bing.com/th/id/R.c3e05885d59b2394023349d79524e15b?rik=XI1YYblpjH3CWA&riu=http%3a%2f%2fwww.pngall.com%2fwp-content%2fuploads%2f2016%2f04%2fDiamond-PNG-Picture-180x180.png&ehk=RrgrG%2bFtGUN%2fNHZfOy34pm6v01Ok6YtfUHAYOgYT14c%3d&risl=&pid=ImgRaw&r=0"
            alt="icon"
            width={50}
            height={50}
          />
        </Box>
        <Box sx={{ position: "absolute", top: "70px", left: "50vw" }}>
          {" "}
          <img
            src="https://th.bing.com/th/id/R.c3e05885d59b2394023349d79524e15b?rik=XI1YYblpjH3CWA&riu=http%3a%2f%2fwww.pngall.com%2fwp-content%2fuploads%2f2016%2f04%2fDiamond-PNG-Picture-180x180.png&ehk=RrgrG%2bFtGUN%2fNHZfOy34pm6v01Ok6YtfUHAYOgYT14c%3d&risl=&pid=ImgRaw&r=0"
            alt="icon"
            width={50}
            height={50}
          />
        </Box>
        <Box sx={{ position: "absolute", top: "10px", left: "10vw" }}>
          {" "}
          <img
            src="https://th.bing.com/th/id/R.c3e05885d59b2394023349d79524e15b?rik=XI1YYblpjH3CWA&riu=http%3a%2f%2fwww.pngall.com%2fwp-content%2fuploads%2f2016%2f04%2fDiamond-PNG-Picture-180x180.png&ehk=RrgrG%2bFtGUN%2fNHZfOy34pm6v01Ok6YtfUHAYOgYT14c%3d&risl=&pid=ImgRaw&r=0"
            alt="icon"
            width={50}
            height={50}
          />
        </Box>
        <Box sx={{ position: "absolute", top: "10px", left: "30vw" }}>
          {" "}
          <img
            src="https://th.bing.com/th/id/R.c3e05885d59b2394023349d79524e15b?rik=XI1YYblpjH3CWA&riu=http%3a%2f%2fwww.pngall.com%2fwp-content%2fuploads%2f2016%2f04%2fDiamond-PNG-Picture-180x180.png&ehk=RrgrG%2bFtGUN%2fNHZfOy34pm6v01Ok6YtfUHAYOgYT14c%3d&risl=&pid=ImgRaw&r=0"
            alt="icon"
            width={50}
            height={50}
          />
        </Box>
      </Box>
      <Box sx={{width:"100%" , display:"flex", justifyContent: "center"}}>

      <Box sx={{width:"600px"}}>
          <h2 className="h2">Customer Service
on Your Terms</h2>
        </Box>
      </Box>
      <Box sx={{ height: "100px" }}>
        <Box sx={{ position: "absolute", bottom: "140px", left: "-10vw" }}>
          <img
            src="https://th.bing.com/th/id/R.c3e05885d59b2394023349d79524e15b?rik=XI1YYblpjH3CWA&riu=http%3a%2f%2fwww.pngall.com%2fwp-content%2fuploads%2f2016%2f04%2fDiamond-PNG-Picture-180x180.png&ehk=RrgrG%2bFtGUN%2fNHZfOy34pm6v01Ok6YtfUHAYOgYT14c%3d&risl=&pid=ImgRaw&r=0"
            alt="icon"
            width={50}
            height={50}
          />
        </Box>
        <Box sx={{ position: "absolute", bottom: "80px", left: "10vw" }}>
        <img
            src="https://th.bing.com/th/id/R.c3e05885d59b2394023349d79524e15b?rik=XI1YYblpjH3CWA&riu=http%3a%2f%2fwww.pngall.com%2fwp-content%2fuploads%2f2016%2f04%2fDiamond-PNG-Picture-180x180.png&ehk=RrgrG%2bFtGUN%2fNHZfOy34pm6v01Ok6YtfUHAYOgYT14c%3d&risl=&pid=ImgRaw&r=0"
            alt="icon"
            width={50}
            height={50}
          />
        </Box>
        <Box sx={{ position: "absolute", bottom: "80px", left: "30vw" }}>
        <img
            src="https://th.bing.com/th/id/R.c3e05885d59b2394023349d79524e15b?rik=XI1YYblpjH3CWA&riu=http%3a%2f%2fwww.pngall.com%2fwp-content%2fuploads%2f2016%2f04%2fDiamond-PNG-Picture-180x180.png&ehk=RrgrG%2bFtGUN%2fNHZfOy34pm6v01Ok6YtfUHAYOgYT14c%3d&risl=&pid=ImgRaw&r=0"
            alt="icon"
            width={50}
            height={50}
          />
        </Box>
        <Box sx={{ position: "absolute", bottom: "140px", left: "50vw" }}>
          <img
            src="https://th.bing.com/th/id/R.c3e05885d59b2394023349d79524e15b?rik=XI1YYblpjH3CWA&riu=http%3a%2f%2fwww.pngall.com%2fwp-content%2fuploads%2f2016%2f04%2fDiamond-PNG-Picture-180x180.png&ehk=RrgrG%2bFtGUN%2fNHZfOy34pm6v01Ok6YtfUHAYOgYT14c%3d&risl=&pid=ImgRaw&r=0"
            alt="icon"
            width={50}
            height={50}
          />
        </Box>
      </Box>
    </Box>
  </Box>
  )
}
